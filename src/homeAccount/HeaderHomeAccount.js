    ...
    return (
        <div className="homeAccount__header">
            ...
            <div className="homeAccount__links">
                ...
                <Link to="/nhaxe" className={window.location.href.split('/')[3] == 'nhaxe' && 'onclick_header__nhaxe'}>
                    nhà xe
                </Link>
                <Link
                    to="/cartour"
                    className={window.location.href.split('/')[3] == 'cartour' && 'onclick_header__chuyenxe'}>
                    chuyến xe
                </Link>
                <Link
                    to="/contact"
                    className={window.location.href.split('/')[3] == 'contact' && 'onclick_header__lienhe'}>
                    Liên hệ
                </Link>
                <Link
                    to="/account"
                    className={window.location.href.split('/')[3] == 'account' && 'onclick_header__taikhoan'}>
                    Tài khoản
                </Link>
                <Link to="/admin" className={window.location.href.split('/')[3] == 'admin' && 'onclick_header__Admin'}>
                    Admin
                </Link>
            </div>
            <div className="homeAccount__right">
                <div className={isMenuOpen ? 'homeAccount_imgtrans--hidden' : ''}>
                    <img className="homeAccount__logoNation" src={logoVN} alt="" />

                    <img className="homeAccount__logoNation" src={logoAnh} alt="" onClick={() => console.log('a')} />
                </div>
                <div className={isMenuOpen ? 'homeAccount_imgtrans--hidden' : ''}>
